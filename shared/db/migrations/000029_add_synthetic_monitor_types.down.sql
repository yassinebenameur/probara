-- Remove synthetic monitor types from the timeout check constraint.
...
ALTER TABLE monitors ADD CONSTRAINT monitors_timeout_check CHECK (
    (type = ANY (ARRAY['http'::text, 'ping'::text, 'sip'::text, 'dns'::text])) AND timeout_seconds > 0 AND timeout_seconds < interval_seconds
