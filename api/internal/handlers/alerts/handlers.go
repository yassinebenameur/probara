...
// GetAlertsByPolicy handles GET /api/v1/alert-policies/{id}/alerts
func (h *Handlers) GetAlertsByPolicy(w http.ResponseWriter, r *http.Request) {
	tenantID, err := middleware.GetTenantID(r.Context())
	if err != nil {
		errors.WriteUnauthorizedError(w, "tenant ID not found")
		return
	...
	if err != nil {
		errors.WriteValidationError(w, "invalid policy ID")
		return
	}
	...
	alerts, err := h.service.GetAlertsByPolicy(r.Context(), tenantUUID, policyID, limit)
	if err != nil {
		h.logger.WithFields(map[string]interface{}{
			"error":     err.Error(),
			"tenant_id": tenantID,
			"policy_id": policyID,
		}).Error("Failed to get alerts by policy")
		errors.WriteInternalError(w, "failed to get alerts")
		return
	}
...
// GetMonitorsByPolicy handles GET /api/v1/alert-policies/{id}/monitors
func (h *Handlers) GetMonitorsByPolicy(w http.ResponseWriter, r *http.Request) {
	tenantID, err := middleware.GetTenantID(r.Context())
	if err != nil {
		errors.WriteUnauthorizedError(w, "tenant ID not found")
		return
	...
	monitors, err := h.service.GetMonitorsByPolicy(r.Context(), tenantUUID, policyID)
	if err != nil {
		h.logger.WithFields(map[string]interface{}{
			"error":     err.Error(),
			"tenant_id": tenantID,
			"policy_id": policyID,
		}).Error("Failed to get monitors by policy")
		errors.WriteInternalError(w, "failed to get monitors")
		return
	}
