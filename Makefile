.PHONY: help up down restart logs build build-agent-static clean ps healthcheck migrate test test-cover lint fmt vet start-all stop-all start-all-local stop-all-local restart-all-local dev-start dev-stop
